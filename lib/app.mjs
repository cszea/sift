...
async function trendingRoute(req) {
  ...
  try {
    ...
    const list = (d && d.data && d.data.itemList) || [];
    const cards = list.map((v) => {
      const uid = v.author && (v.author.uniqueId || v.author.unique_id);
      const id = v.id;
      if (!uid || !id) return null;
      return {
        id: "tt_" + id, platform: "tiktok",
        url: `https://www.tiktok.com/@${uid}/video/${id}`,
        title: (v.desc || "").trim().replace(/\s+/g, " ").slice(0, 120) || "Trending on TikTok",
        author: "@" + uid, desc: "Trending on TikTok — swipe → keep.",
        submittedBy: "TikTok trending", trending: true
      };
    }).filter(Boolean);
