...
export async function handleRequest(req, store) {
  ...
  try {
    ...
    if (path === "/api/trending") return await trendingRoute(req);
    if (path === "/api/home-feed") return await homeFeedRoute();
    if (path === "/api/similar") return await similarRoute(req, store);
...
async function bulkImportRoute(req, store, auth) {
  ...
  const s = await getState(store);
  const now = Date.now();
  let addedFeed = 0, addedBank = 0;
  for (const it of items) {
    const url = String((it && it.url) || "").trim();
    if (!/^https?:\/\//i.test(url)) continue;
    const fmt = FORMATS.includes(it && it.format) ? it.format : undefined;
    const card = {
      id: rid(), platform: detectPlatform(url), url, format: fmt,
      title: (it && String(it.title || "").trim()) || "Imported reference",
      author: "", desc: "Bulk-imported — open to preview.", img: "",
      submittedBy: auth.name || auth.email, submittedByEmail: auth.email, submittedAt: now
    };
    if (toBank && fmt) { (s.bank[fmt] || (s.bank[fmt] = [])).unshift({ ...card, savedAt: now }); addedBank++; }
    else { s.feed = [card, ...(Array.isArray(s.feed) ? s.feed : [])]; addedFeed++; }
  }
  await setState(store, s);
  const total = addedFeed + addedBank;
  if (total > 0) await incStat(store, auth.email, auth.name || auth.email, "imports", total);
  return json({ ok: true, addedFeed, addedBank, total });
}
...
/* map a TikHub itemList to Sift cards */
function mapTikTokItems(list, label, submittedBy) {
  return (list || []).map((v) => {
    const uid = v.author && (v.author.uniqueId || v.author.unique_id), id = v.id;
    if (!uid || !id) return null;
    return {
      id: "tt_" + id, platform: "tiktok", url: `https://www.tiktok.com/@${uid}/video/${id}`,
      title: (v.desc || "").trim().replace(/\s+/g, " ").slice(0, 120) || label,
      author: "@" + uid, desc: label + " — swipe → keep.", submittedBy: submittedBy || label, trending: true
    };
  }).filter(Boolean);
}

/* --- live trending TikToks via TikHub (For You feed) --- */
async function trendingRoute(req) {
  const key = process.env.TIKHUB_KEY;
  if (!key) return json({ cards: [], configured: false });
  const url = new URL(req.url);
  const count = Math.min(30, Math.max(1, parseInt(url.searchParams.get("count"), 10) || 15));
  try {
    const r = await fetch(`https://api.tikhub.io/api/v1/tiktok/web/fetch_explore_post?count=${count}`, {
      headers: { Authorization: "Bearer " + key, "User-Agent": "Sift/1.0" },
      signal: AbortSignal.timeout(22000)
    });
    const d = await r.json().catch(() => ({}));
    const cards = mapTikTokItems((d && d.data && d.data.itemList) || [], "Trending on TikTok", "TikTok trending");
    return json({ cards, configured: true });
  } catch (e) {
    return json({ cards: [], configured: true, error: String(e.message || e) });
  }
}

/* --- a specific account's personalized For You feed (needs its cookie) --- */
async function homeFeedRoute() {
  const key = process.env.TIKHUB_KEY;
  const cookie = process.env.TIKTOK_COOKIE;
  if (!key) return json({ cards: [], configured: false });
  if (!cookie) return json({ cards: [], configured: true, needCookie: true });
  try {
    const r = await fetch("https://api.tikhub.io/api/v1/tiktok/web/fetch_home_feed", {
      method: "POST",
      headers: { Authorization: "Bearer " + key, "content-type": "application/json", "User-Agent": "Sift/1.0" },
      body: JSON.stringify({ count: 15, cookie }),
      signal: AbortSignal.timeout(25000)
    });
    const d = await r.json().catch(() => ({}));
    const cards = mapTikTokItems((d && d.data && d.data.itemList) || [], "From your For You", "Your For You");
    return json({ cards, configured: true, personalized: true });
  } catch (e) {
    return json({ cards: [], configured: true, error: String(e.message || e) });
  }
}

/* --- "similar" feed: more videos from the creators the team has submitted --- */
async function similarRoute(req, store) {
  const key = process.env.TIKHUB_KEY;
  if (!key) return json({ cards: [], configured: false });
  const s = await getState(store);
  const pool = [...(s.feed || []), ...Object.values(s.bank || {}).flat(), ...Object.values(s.archive || {}).flat(), ...(s.passed || [])];
  const users = [];
  for (const c of pool) {
    if (!c || !c.url) continue;
    const m = String(c.url).match(/tiktok\.com\/@([\w.\-]+)\/video/i);
    if (m && !users.includes(m[1])) users.push(m[1]);
  }
  const pick = users.slice(0, 4); // cap TikHub calls
  if (!pick.length) return json({ cards: [], configured: true, noCreators: true });
  const H = { Authorization: "Bearer " + key, "User-Agent": "Sift/1.0" };
  const cards = [], seen = new Set();
  for (const uid of pick) {
    try {
      const prof = await (await fetch(`https://api.tikhub.io/api/v1/tiktok/web/fetch_user_profile?uniqueId=${encodeURIComponent(uid)}`, { headers: H, signal: AbortSignal.timeout(15000) })).json();
      const sec = prof && prof.data && prof.data.userInfo && prof.data.userInfo.user && prof.data.userInfo.user.secUid;
      if (!sec) continue;
      const posts = await (await fetch(`https://api.tikhub.io/api/v1/tiktok/web/fetch_user_post?secUid=${encodeURIComponent(sec)}&count=6`, { headers: H, signal: AbortSignal.timeout(15000) })).json();
      const list = (posts && posts.data && posts.data.itemList) || [];
      for (const v of list) {
        const au = v.author && (v.author.uniqueId || v.author.unique_id), id = v.id;
        if (!au || !id || seen.has(id)) continue;
        seen.add(id);
        cards.push({ id: "tt_" + id, platform: "tiktok", url: `https://www.tiktok.com/@${au}/video/${id}`, title: (v.desc || "").trim().replace(/\s+/g, " ").slice(0, 120) || ("More from @" + au), author: "@" + au, desc: "Similar — more from @" + au, submittedBy: "Similar · @" + au, trending: true });
      }
    } catch (e) { /* skip creator on error */ }
  }
  return json({ cards: cards.slice(0, 18), configured: true, creators: pick });
}
